...
#ifndef __CAConsole_H__
#define __CAConsole_H__
/// @cond DO_NOT_SHOW
...
/** CAConsole is helper class that lets the developer control the game from TCP connection.
 CAConsole will spawn a new thread that will listen to a specified TCP port.
 CAConsole has a basic token parser. Each token is associated with an std::function<void(int)>.
...
public:
    ...
    public:
        ...
        /** send prompt string to CAConsole */
    ...
    /** stops the CAConsole. 'stop' will be called at destruction time as well */
    void stop();
    ...
    /** log something in the CAConsole */
...
protected:
    ...
    void createCommandCAApplication();
    ...
    void commandCAApplicationSubCommandPause(int fd, const std::string& args);
    void commandCAApplicationSubCommandResume(int fd, const std::string& args);
    void commandCAApplicationSubCommandStop(int fd, const std::string& args);
    void commandCAApplicationSubCommandStart(int fd, const std::string& args);
    void commandCAApplicationSubCommandEnd(int fd, const std::string& args);
    ...
    // file descriptor: socket, CAConsole, etc.
    int _listenfd;
    int _maxfd;
    std::vector<int> _fds;
    std::thread _thread;
    ...
    // strings generated by cocos2d sent to the remote CAConsole
...
private:
    CC_DISALLOW_COPY_AND_ASSIGN(CAConsole);
    
    // helper functions
    void printFileUtils(int fd);
    
    /** send help message to CAConsole */
...
/// @endcond
#endif /* defined(__CCCAConsole_H__) */
